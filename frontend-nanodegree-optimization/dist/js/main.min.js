...
var adjectives="dark color whimsical shiny noisy apocalyptic insulting praise scientific".split(" "),nouns="animals everyday fantasy gross horror jewelry places scifi".split(" ");function generator(a,b){var d=getAdj(a),c=getNoun(b),e=parseInt(Math.random()*d.length),f=parseInt(Math.random()*c.length);return"The "+d[e].capitalize()+" "+c[f].capitalize()}
...
function(){return pizzaIngredients.crusts[Math.floor(Math.random()*pizzaIngredients.crusts.length)]},ingredientItemizer=function(a){return"<li>"+a+"</li>"},makeRandomPizza=function(){for(var a="",b=Math.floor(4*Math.random()),d=Math.floor(3*Math.random()),c=Math.floor(2*Math.random()),e=0;e<b;e++)a+=ingredientItemizer(selectRandomMeat());for(b=0;b<d;b++)a+=ingredientItemizer(selectRandomNonMeat());for(d=0;d<c;d++)a+=ingredientItemizer(selectRandomCheese());a+=ingredientItemizer(selectRandomSauce());
return a+ingredientItemizer(selectRandomCrust())},pizzaElementGenerator=function(a){var b=document.createElement("div"),d=document.createElement("div"),c=document.createElement("img"),e=document.createElement("div");b.classList.add("randomPizzaContainer");b.style.width="33.33%";b.style.height="325px";b.id="pizza"+a;d.style.width="35%";c.src="img/pizza.png";c.classList.add("img-responsive");d.appendChild(c);b.appendChild(d);e.style.width="65%";a=document.createElement("h4");a.innerHTML=randomName();
e.appendChild(a);a=document.createElement("ul");a.innerHTML=makeRandomPizza();e.appendChild(a);b.appendChild(e);return b},resizePizzas=function(a){window.performance.mark("mark_start_resize");a:switch(a){case "1":document.getElementById("pizzaSize").innerHTML="Small";break a;case "2":document.getElementById("pizzaSize").innerHTML="Medium";break a;case "3":document.getElementById("pizzaSize").innerHTML="Large";break a;default:console.log("bug in changeSliderLabel")}var b=document.getElementsByClassName("randomPizzaContainer"),
d=b.length,c=function(a,b){var c=a.offsetWidth,d=document.getElementById("randomPizzas").offsetWidth;c/=d;a:{switch(b){case "1":var e=.25;break a;case "2":e=.3333;break a;case "3":e=.5;break a;default:console.log("bug in sizeSwitcher")}e=void 0}return(e-c)*d}(b[0],a),e=b[0].offsetWidth+c+"px";(function(a){for(a=0;a<d;a++)b[a].style.width=e})(a);window.performance.mark("mark_end_resize");window.performance.measure("measure_pizza_resize","mark_start_resize","mark_end_resize");a=window.performance.getEntriesByName("measure_pizza_resize");
console.log("Time to resize pizzas: "+a[a.length-1].duration+"ms")};window.performance.mark("mark_start_generating");for(var pizzasDiv=document.getElementById("randomPizzas"),i=2;100>i;i++)pizzasDiv.appendChild(pizzaElementGenerator(i));window.performance.mark("mark_end_generating");window.performance.measure("measure_pizza_generation","mark_start_generating","mark_end_generating");var timeToGenerate=window.performance.getEntriesByName("measure_pizza_generation");
console.log("Time to generate pizzas on load: "+timeToGenerate[0].duration+"ms");var frame=0;function logAverageFrame(a){for(var b=a.length,d=0,c=b-1;c>b-11;c--)d+=a[c].duration;console.log("Average scripting time to generate last 10 frames: "+d/10+"ms")}var lastScrollY=0,ticking=!0;function onScroll(){lastScrollY=window.scrollY;requestTick()}function requestTick(){ticking||requestAnimationFrame(updatePositions);ticking=!0}
function updatePositions(){ticking=!1;frame++;window.performance.mark("mark_start_frame");for(var a=document.getElementsByClassName("mover"),b=lastScrollY/1250,d,c=0;c<a.length;c++)d=Math.sin(b+c%5),a[c].style.left=a[c].basicLeft+100*d+"px";window.performance.mark("mark_end_frame");window.performance.measure("measure_frame_duration","mark_start_frame","mark_end_frame");0===frame%10&&(a=window.performance.getEntriesByName("measure_frame_duration"),logAverageFrame(a))}
window.addEventListener("scroll",onScroll);document.addEventListener("DOMContentLoaded",function(){for(var a=0;36>a;a++){var b=document.createElement("img");b.className="mover";b.src="img/pizza.png";b.style.height="100px";b.style.width="73.333px";b.basicLeft=a%8*256;b.style.top=256*Math.floor(a/8)+"px";document.getElementById("movingPizzas1").appendChild(b)}updatePositions()});
